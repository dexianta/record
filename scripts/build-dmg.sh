...
VERSION="${RECORD_VERSION:-$(plutil -extract CFBundleShortVersionString raw "$PROJECT_DIR/Packaging/Info.plist")}"
BUILD_NUMBER="${RECORD_BUILD_NUMBER:-$(plutil -extract CFBundleVersion raw "$PROJECT_DIR/Packaging/Info.plist")}"
UPDATE_PATH="$DIST_DIR/record-$VERSION.zip"
...
codesign --force --deep --sign "$SIGN_IDENTITY" "$APP_PATH/Contents/Frameworks/Sparkle.framework"
codesign --force --sign "$SIGN_IDENTITY" "$APP_PATH"
codesign --verify --deep --strict --verbose=2 "$APP_PATH"

rm -f -- "$UPDATE_PATH"
ditto -c -k --sequesterRsrc --keepParent "$APP_PATH" "$UPDATE_PATH"
